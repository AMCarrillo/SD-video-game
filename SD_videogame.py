...
#The speed is in pixels per iteration
class Soldier (pygame.sprite.Sprite):
	def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
		...
		self.direction = 1 #1 means right
		self.vel_y = 0
		self.jump = False
		self.in_air = True
		self.flip = False
		...
		self.move_counter = 0
		self.vision = pygame.Rect(0, 0, 150, 20)#x, y, w, h
		self.idling = False
	...
	def move(self, moving_left, moving_right):
		...
		#check collision in the y direction
			if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
				#check if below the ground, i.e. jumpig
				if self.vel_y < 0:
					self.vel_y = 0
